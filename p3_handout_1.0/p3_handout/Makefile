...
OBJECTS=$(addprefix $(OBJDIR)/,hiy.o logging.o parse.o util.o)

#--------------------------------------------------------------------
# Build Recipies for the Executables (binary)
#--------------------------------------------------------------------
all: hiy my_pause slow_cooker my_echo

hiy: $(OBJECTS) 
	$(CC) $(CFLAGS) -o $@ $^
#	gcc -Wall -std=gnu11 -o hiy hiy.o logging.o parse.o util.o

$(OBJDIR)/hiy.o: $(SRCDIR)/hiy.c $(INCDIR)/hiy.h
	$(CC) -c $(CFLAGS) -o $@ $<
...
clean:
	rm -rf $(OBJDIR)/*.o hiy my_pause slow_cooker my_echo
